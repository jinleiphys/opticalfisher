...
4. *It delivers actionable experimental guidance.* The step-by-step decomposition (Fig. 4) and global analysis (Fig. 5) provide a clear priority ordering for rare-isotope beam facilities: multi-energy systematics $\gg$ $A_y$ $\gg$ $\sigma_R$ (at typical precision ratios; the quantitative $A_y$ gain depends on $\epsilon/\delta_{A_y}$, see End Matter). This is immediately relevant to experimental programs at FRIB, RIKEN, and other facilities where beam time allocation requires quantitative justification.
