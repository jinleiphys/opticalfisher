...
I have now extended the analysis to include three of the four scattering-region observable types the referee lists: elastic angular distributions, analyzing powers $A_y$, and total cross sections $\sigma_T$ (plus the reaction cross section $\sigma_R$). The fourth type, average resonance parameters, constrains the potential in the few-MeV resonance region, which lies below the 10--200 MeV continuum regime of the present analysis. In that regime, individual compound-nucleus resonances dominate and smooth optical potentials such as KD02 are not designed to apply, as their Woods-Saxon forms cannot capture the rapid energy dependence near individual resonances. The revised manuscript now explicitly states this scope limitation (Introduction, paragraph 2). In principle, the Fisher framework can accommodate resonance data by including their contribution in the Fisher matrix (schematically, $F_\mathrm{total} = F_\mathrm{scattering} + F_\mathrm{resonance}$), provided both are evaluated in the same parameter space with a consistent error model; this extension is left for future work. The revised manuscript presents a step-by-step decomposition (new Fig. 4):
...
- **Step 3: $+ A_y$**: Adding $A_y$ raises $D_\mathrm{eff}$ from 1.22 to 1.53 for $n+{}^{40}$Ca, and from 1.15 to 1.72 for $n+{}^{120}$Sn (at sensitivity normalization $\epsilon = 1$, $\delta_{A_y} = 0.03$). The improvement is entirely due to $A_y$, which provides genuinely new information through the spin-flip amplitude $g(\theta)$ probing spin-orbit interference. Within the present absolutely-normalized setup, integrated cross sections ($\sigma_R$, $\sigma_T$) provide no measurable additional gain, as they are already captured by the angular distribution. The quantitative gain from $A_y$ depends on the relative precision ratio $\epsilon/\delta_{A_y}$: at typical experimental elastic precision ($\epsilon = 5\%$--$10\%$), the gain ranges from ~5% to ~35% across systems; at $\epsilon = 1$ (equal statistical weight), the gain is ~25%--50%. The qualitative hierarchy multi-energy $\gg$ $A_y$ $\gg$ $\sigma_R$ holds at all precision ratios tested ($\epsilon = 1\%$--$100\%$). See End Matter, "Robustness checks."
- **Step 4: Multi-energy (7 energies)**: With elastic data alone, multi-energy raises $D_\mathrm{eff}$ from 1.22 to 2.52 for $n+{}^{40}$Ca and from 1.15 to 2.33 for $n+{}^{120}$Sn, the largest single improvement from the elastic baseline. Including all observables gives $D_\mathrm{eff} = 2.18$ (Ca) and 1.81 (Sn), somewhat lower because $A_y$ concentrates information along already-stiff directions. The all-observable multi-energy average across 24 systems is $2.4 \pm 0.4$; elastic-only multi-energy gives higher values ($\sim$2.3--2.8 in representative cases) because $A_y$ sharpens already-stiff directions rather than opening new ones.
...
The key new insight, as the referee anticipated, is precisely this: even after including all scattering-region observables available within 10--200 MeV ($d\sigma/d\Omega$, $A_y$, $\sigma_T$, $\sigma_R$) and combining 7 energies, $D_\mathrm{eff}$ reaches only $\sim$2.4 $\pm$ 0.4 (all-observable average across 24 systems) in the 13-parameter local space (and $D_\mathrm{eff} \approx 3.0$ out of 48 in the universal KD02 coefficient space), far below the number of model parameters. Within the KD02 framework, the information limit appears to be a robust feature of how elastic scattering encodes potential information. The step-by-step decomposition reveals *which* observables contribute *what*: $A_y$ lifts the spin-orbit degeneracy, multi-energy data lift the imaginary-parameter degeneracy, but single-energy $\sigma_R$ provides negligible additional constraint.

**Physical picture of the Igo ambiguity** (revised Fig. 3, Discussion paragraph 1): The eigenvector analysis clarifies the classical Igo ambiguity. The dominant eigenvector $\mathbf{e}_1$ is a low-dimensional subspace dominated by $r_v$ (62%), with contributions from $r_d$ (14%) and $V$ (11%), all entering with the same sign. This corresponds to the volume integral direction $J_V \propto V r_v^3$: increasing all three together produces a measurably different cross section. The Igo ambiguity is the orthogonal "sloppy" direction within this subspace, where $V$ increases while $r_v$ decreases (or vice versa) so that $J_V$ stays approximately constant. The eigenvalue ratio between these directions exceeds $10^5$, making the depth-radius tradeoff effectively unmeasurable from elastic scattering alone.
...
**(i) Multi-energy combined Fisher analysis**: The combined Fisher matrix is constructed by first mapping each single-energy $F(E)$ to the shared KD02 global coefficient space via the Jacobian $J(E) = \partial\boldsymbol{\theta}_\mathrm{local}(E)/\partial\boldsymbol{\theta}_\mathrm{global}$, summing $F_\mathrm{global} = \sum_E J^T F(E) J$, and projecting back to the 13-dimensional local space at a reference energy. This captures the information from measurements across 7 energies (10--200 MeV). Combining elastic data alone across energies raises $D_\mathrm{eff}$ from ~1.2 to ~2.4 (average across 24 nucleus-projectile combinations) in the 13-parameter local space (new Fig. 4, Step 4). The energy lever arm is the key: at low energies the real potential dominates, while at high energies absorption becomes stronger, probing orthogonal parameter combinations. This gain relies jointly on the additional data *and* the KD02 analytical energy dependence that ties parameters across energies.
...
1. The Fisher information matrix rationalizes the empirical findings of King *et al.* (End Matter, "Connection to Bayesian analysis"): in the Gaussian (Laplace) approximation, the posterior covariance equals $F^{-1}$, so the $D_\mathrm{eff} \approx 1.8$ found here is consistent with why only $V$ and $r_v$ showed strong correlation in their MCMC scatter plots. Specifically, $V$ and $r_v$ both enter the dominant eigenvector $\mathbf{e}_1$ with the same sign (the volume integral direction), producing the elongated correlation structure observed in the MCMC posterior.
2. The FIM eigenvalue hierarchy (condition number $>10^7$) is consistent with the elongated Bayesian posterior and clarifies why the frequentist $\chi^2$ approach produced spurious correlations (End Matter, "Relation to Bayesian analysis").
3. The Fisher approach is ~3700x more efficient ($2N_p + 1 = 27$ evaluations for $N_p = 13$ vs. ~100,000 MCMC samples), enabling the systematic 168-configuration scan that would be prohibitively expensive with MCMC (End Matter, "Computational advantages").
4. The scale-invariance of $D_\mathrm{eff}$ is an exact mathematical property of the participation ratio, a result that would be difficult to establish empirically from finite MCMC chains.

The two approaches are complementary: Fisher efficiently maps constraint structure and identifies scale-invariant properties, while Bayesian MCMC captures non-Gaussian posterior features (Discussion, paragraph 2 of the main text; End Matter, "Relation to Bayesian analysis").
...
- Added observables: analyzing power $A_y$ (via spin-flip amplitude $g(\theta)$), reaction cross section $\sigma_R$, and total cross section $\sigma_T$

### New analyses
- **Step-by-step constraint decomposition** (new Fig. 4; main text, "Step-by-step constraint analysis"): progressively adds $\sigma_R$, $A_y$, $\sigma_T$, and multi-energy data, quantifying the information contribution of each
- **Multi-energy combined Fisher analysis** (7 energies, 10--200 MeV): $D_\mathrm{eff}$ increases from $\sim$1.2 to $\sim$2.4 (average) in the 13-parameter local space, through Jacobian projection via KD02 energy dependence
...
- **Fisher--Bayesian comparison** (End Matter, "Connection to Bayesian analysis" and "Relation to Bayesian analysis"): connects to King *et al.* [PRL **122**, 232502 (2019)], showing that the FIM eigenvector structure rationalizes their MCMC correlation patterns
