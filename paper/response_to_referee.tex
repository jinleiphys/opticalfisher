...
The per-nucleus analysis in the universal space shows $D_\mathrm{eff} = 1.3$--$3.2$ (all 7 energies combined), with lighter nuclei like $^{12}$C having higher values. These results are presented in the revised End Matter with a new figure (Fig.~5) showing the D$_\mathrm{eff}$ saturation, eigenvalue spectrum, and eigenvector composition in the 45-dimensional universal coefficient space.
...
\item New step-by-step constraint analysis decomposing the information contribution of each observable and multi-energy data (new Fig.~4)
\item New angle-resolved sensitivity analysis showing $|S_i(\theta)|$ for all 11 parameters (new Fig.~7)
