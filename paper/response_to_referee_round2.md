...
The clause "sloppy parameter combinations vary by orders of magnitude" describes what the data permit, not what published fits display. In the local Fisher metric, the data permit uncertainties orders of magnitude larger along the sloppy directions than along the stiff ones; the manuscript states the optical-model version of this quantitatively in the End Matter:
...
Referee B does not identify a numerical inconsistency in the Fisher analysis. The report's concerns are now addressed explicitly: the Fisher-spectrum meaning of "sloppy" is stated in the Introduction, and the role of external physical constraints is stated in the Discussion. The remaining disagreement, I believe, concerns the breadth of interpretation rather than the analysis itself. The revised manuscript is scoped to KD02 throughout, and within that scope the result is a compact and broadly useful statement about information limits in optical-potential extraction.
