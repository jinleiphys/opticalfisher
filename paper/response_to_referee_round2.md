...
I agree with the physics history the Referee summarizes, and the manuscript is built on the same picture. The key distinction is not whether these constraints exist; they do. The distinction is whether they come from a single-energy elastic angular distribution alone. Radii and diffuseness constraints import density, bound-state, or dispersive information, while depth constraints from wide-energy-range systematics are precisely the multi-energy information source quantified in Step 4. That the community needs these inputs is the manuscript's thesis: a single-energy angular distribution by itself determines about 1.7 parameter combinations, and the rest of what we know about the parameters comes from additional measurements, theory, and systematics. Moreover, the mapping from these external inputs to Woods-Saxon radii, diffusenesses, and depths is conditional on the assumed density, folding interaction, dispersive ansatz, or global energy dependence.
...
| 4 | Scope of conclusions and venue (B) | KD02 scoping statements retained throughout the manuscript; the separation between single-energy angular distributions, multi-energy systematics, external physical constraints, and downstream reaction-model error attribution is stated in Point 4 of this reply, and PRL relevance is stated explicitly |
